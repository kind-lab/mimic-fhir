...
DROP TABLE IF EXISTS mimic_fhir.medication_statement;
CREATE TABLE mimic_fhir.medication_statement(
    id          uuid PRIMARY KEY,
...
WITH fhir_medication_statement AS (
    SELECT 
...
INSERT INTO mimic_fhir.medication_statement
SELECT
    ...
    , jsonb_strip_nulls(jsonb_build_object(
        ...
        , 'medicationCodeableConcept', jsonb_build_array(jsonb_build_object(
            'coding', jsonb_build_array(
                ...
                ELSE NULL END,
                CASE WHEN med_ETC_CODES IS NOT NULL THEN med_ETC_CODES ELSE NULL END
            )
        ))
